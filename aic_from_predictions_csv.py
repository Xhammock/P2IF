...
def count_lightgbm_trees(model_txt_path: str) -> int:
    """In LightGBM save_model text format, each root tree starts with a 'Tree=i' line."""
...
def main() -> None:
    ...
    p.add_argument("--csv", required=True, help="CSV with y_true and y_pred (prefer train_predictions.csv)")
    ...
    k_group.add_argument(
        "--k",
        type=int,
        help="Parameter count k (e.g. linear model: n_features + intercept; declare meaning in manuscript)",
    )
    k_group.add_argument(
    ...
    p.add_argument("--label", default="", help="Optional model name tag for printed output")
    args = p.parse_args()
